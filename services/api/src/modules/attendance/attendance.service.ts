...
export async function svcGetAttendance() {
    if (isMongoConnected) {
      try {
        return await MongoAttendance.find({}).sort({ date: -1 });
      } catch (err) {
        console.error('Mongo get attendance error:', err);
      }
    }
    const db = getDb();
    return [...db.attendance].sort((a, b) => b.date.localeCompare(a.date));
}

export async function svcGetUserAttendance(employeeId: string) {
  if (isMongoConnected) {
    try {
      return await MongoAttendance.find({ employeeId }).sort({ date: -1 });
    }
    catch (err) {
...
export async function svcGetEmployeeMetrics(employeeId: string) {
...
export async function svcGetAdminMetrics() {
  const todayStr = dayjs().format('YYYY-MM-DD');
  const today = dayjs();
...
export async function svcCheckIn(body: any, files: Express.Multer.File[]) {

  const cleanupFiles = (fileList: Express.Multer.File[] | undefined) => {
    if (fileList && Array.isArray(files)) {
      fileList.forEach(f => {
  ...
  if (!files || files.length === 0) {
    throw { status: 400, message: 'Attendance selfie image is mandatory for face verification.', };
  }
  ...
  if (alreadyExists) {
    cleanupFiles(files);
    throw { status: 400, message: 'You have already checked in today.' };
  }

  if (!user) {
    cleanupFiles(files);
    throw { status: 404, message: 'Employee record not found.' };
  }

  if (!user.avatarUrl) {
    cleanupFiles(files);
    throw { status: 400, message: 'Face Match Failed: You do not have a registered profile picture (avatar) in your account. A mandatory profile image is required for biometric face comparison. Please contact an administrator to upload one.' };
  }
  ...
  if (!isSimulated && externalPipelineUrl && externalPipelineUrl.trim() !== '' && externalPipelineUrl !== 'undefined') {
    ...
    try {
      ...
      if (response.data.status === 'spoof_suspected') {
        cleanupFiles(files);
        throw { status: 400, message: `Spoof Detected: ${response.data.message || 'Liveness check failed.'}`, };
      }
      ...
      if (apiErr.response) {
        ...
        throw {
          ...
          message: `Face Match Failed: ${apiErrorMsg}`
        };
      }
  ...
  if (!isMatch) {
    throw { status: 400, message: `Face Match Mismatch: Biometric matching confidence (${similarityConfidence}%) fell below the required threshold of 90%. Please align your face carefully at the requested angle (${targetAngle}°) and retry.` };
  }
  ...
  if (latitude && longitude) {
    ...
    const q1 = (latitude * Math.PI) / 180;
    const q2 = (longitude * Math.PI) / 180;
    const dq = ((longitude - latitude) * Math.PI) / 180;
    const dl = ((settings.longitude - settings.latitude) * Math.PI) / 180;
    const a =
  ...
  if (isMongoConnected) {
    try {
      ...
      throw { status: 500, message: 'Database saving error during check-in' };
    }
  }
  else {
    const db = getDb();
...
export async function svcCheckOut(body: any) {
  ...
  if (isMongoConnected) {
    try {
      const record = await MongoAttendance.findOne({ employeeId, date: todayStr });
      if (!record) {
        throw { status: 400, message: 'No check-in record found for today. Please check-in first.' };
      }
      if (record.checkOut) {
        throw { status: 400, message: 'You have already checked out today.' };
      }
      ...
      throw { status: 500, message: 'Database saving error during check-out' };
    }
  }
